...
vec3 PointLightContribution(int light_num) {
	...
	vec3 ambient = DiffuseColor * point_light_ambient[light_num];
        vec3 diffuse = diffuseShade * DiffuseColor;
        vec3 specular = specularShade * SpecularColor;
        ...
        return point_light_intensity[light_num] * (attenuation * (diffuse + specular) + ambient);
}
